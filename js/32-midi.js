...
const GM_PROG={piano:0,epiano:4,organ:19,pluck:46,guitar:24,eguitar:26,dist:30,bell:14,marimba:12,strings:48,cello:42,
  pad:89,lead:80,supersaw:81,brass:62,bass:33,subbass:38,acid:38,musicbox:10,vibes:11,koto:107,choir:52,flute:73,plucksyn:99};
...
function exportMidi(){
  ...
  for(const tr of state.tracks){
    ...
    if(tr.kind==='inst'){
      const hold=arpOn(tr)?arpLenOf(tr):1;         // 琶音音长（格）：note off 随之推后
      for(let s=0;s<n;s++){
        const fr=followRow(tr,s);                  // 实际发声行：琶音只在画的 step 上发声（空步返回 -1）
        if(fr==null||fr<0) continue;               // 没画 / 不触发 → 不发声（与播放路径同一判定）
        ...
        ev.push({tick:Math.min(s+hold,n)*stepT,seq:0,bytes:[0x80|ch,nte,0]});   // 音长 1/8·1/4 时音符延长（clamp 到本声部末尾）
      }
    }else{
